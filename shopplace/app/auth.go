...
func loginHandler(w http.ResponseWriter, r *http.Request) *appError {
	...
	//go func() {
	//_ = testTmpl.Execute(w, r, session.Values[oauthFlowRedirectKey])
	//}()
...
func oauthCallbackHandler(w http.ResponseWriter, r *http.Request) *appError {
	...
	if !ok {
		return appErrorf(err, "invalid state parameter stage2. try logging in again... redirectURL= "+redirectURL)
	}
