...
var (
	...
	testTmpl   = parseTemplate("test.html")
)
...
//ListData is the Data used to render the list.html, it wraps the shops (Dshops) and
//url path (SignupURL) used in list.html rendering
type ListData struct {
	Dshops    []*shopplace.Shop
	SignupURL string
}
...
func listHandler(w http.ResponseWriter, r *http.Request) *appError {
	...
	var data ListData
	data.Dshops = shops
	data.SignupURL = "/login?redirect=/shops/add"

	return listTmpl.Execute(w, r, data)
...
//DetailData is the Data used to render the list.html, it wraps the shops (Dshops) and
//url path (SignupURL) used in list.html rendering
type DetailData struct {
	Dshop  *shopplace.Shop
	IsUser bool
}
...
func detailHandler(w http.ResponseWriter, r *http.Request) *appError {
	...
	//_ = testTmpl.Execute(w, r, data)
	//return nil
	return detailTmpl.Execute(w, r, data)
...
func editFormHandler(w http.ResponseWriter, r *http.Request) *appError {
	...
	if user != nil {
		...
		if shop.CreatedByID == user.Id {
			return editTmpl.Execute(w, r, shop)
		}
		//var aError *appError
		aError := &appError{nil, "User Attempting to Edit not Owned shop is prohibited", 1}
		return aError
	}
	// Not Logged in.
...
func deleteHandler(w http.ResponseWriter, r *http.Request) *appError {
	...
	if user != nil {
		...
		shop, err := shopplace.DB.GetShop(id)
		if err != nil {
			aError := &appError{err, "User Attempting to Delete not Owned shop is prohibited", 1}
			return aError
		}
		if shop.CreatedByID == user.Id {
		...
		aError := &appError{nil, "User Attempting to Delete not Owned shop is prohibited!!! if you are sure this is your shop please Login and try again", 1}
		return aError
	}
	//Not login
	http.Redirect(w, r, "/", http.StatusFound)
