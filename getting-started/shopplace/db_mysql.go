...
// mysqlDB persists shops to a MySQL instance.
type mysqlDB struct {
	conn *sql.DB
...
// Ensure mysqlDB conforms to the ShopDatabase interface.
var _ ShopDatabase = &mysqlDB{}
...
// newMySQLDB creates a new ShopDatabase backed by a given MySQL server.
func newMySQLDB(config MySQLConfig) (ShopDatabase, error) {
...
// scanShop reads a shop from a sql.Row or sql.Rows
func scanShop(s rowScanner) (*Shop, error) {
	var (
		id            int64
...
const listStatement = `SELECT * FROM shops ORDER BY title`

// ListShops returns a list of shops, ordered by title.
func (db *mysqlDB) ListShops() ([]*Shop, error) {
...
func (db *mysqlDB) ListShopsCreatedBy(userID string) ([]*Shop, error) {
	if userID == "" {
		return db.ListShops()
	}
...
const getStatement = "SELECT * FROM shops WHERE id = ?"

// GetShop retrieves a shop by its ID.
func (db *mysqlDB) GetShop(id int64) (*Shop, error) {
	shop, err := scanShop(db.get.QueryRow(id))
	if err == sql.ErrNoRows {
		return nil, fmt.Errorf("mysql: could not find shop with id %d", id)
	}
	if err != nil {
		return nil, fmt.Errorf("mysql: could not get shop: %v", err)
	}
	return shop, nil
}
...
func (db *mysqlDB) AddShop(b *Shop) (id int64, err error) {
...
const deleteStatement = `DELETE FROM shops WHERE id = ?`

// DeleteShop removes a given shop by its ID.
func (db *mysqlDB) DeleteShop(id int64) error {
	if id == 0 {
...
func (db *mysqlDB) UpdateShop(b *Shop) error {
	if b.ID == 0 {
...
func (config MySQLConfig) ensureTableExists() error {
	...
	if _, err := conn.Exec("DESCRIBE shops"); err != nil {
